...
from base_common.data_format import DataFormat
...
class IOSDevice(IOSBase):
    ...
    def screenshot_and_save(self, filename=None):
        ...
        try:
            ...
            img_path = os.path.join(os.path.join(os.getcwd(), 'image.temp'), img_name)
            remote = "http://{0}:8081/api/common/image?image_id={1}".format(ip_config.get_ip(), img_name)
...
class IOSApp(IOSBase):
    ...
    def open_app(self, app_bundle_id):
        ...
        if self.device is None:
            ...
            try:
                self.app = self.device.session(app_bundle_id)
                self.__app_init()
...
class IOSAssert(IOSApp):
    ...
    def element_exists(self, element, find_type=''):
        """
        元素是否存在
        """
        el = self.find_element(element, find_type)
